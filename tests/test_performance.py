...
from app.default_transforms import clean_data, feature_engineering, transform
...
def test_clean_data_performance(benchmark, sample_data):
    ...
    config = {"remove_duplicates": True, "missing_strategy": "fill", "standardize_text": True}
...
def test_feature_engineering_performance(benchmark, sample_data):
    ...
    config = {"add_text_features": True, "add_numeric_features": True, "add_date_features": True}
...
def test_full_transform_performance(benchmark, sample_data):
    ...
    config = {"clean_data": True, "feature_engineering": True, "add_metadata": True}
...
def test_chunked_processing_performance(benchmark, large_data):
    ...
    config = {"chunk_threshold": 5000, "chunk_size": 2500, "clean_data": True}
...
def test_scalability(data_size):
    ...
    np.random.seed(42)
    data = pd.DataFrame(
        {"id": range(data_size), "value": np.random.randn(data_size), "category": np.random.choice(["A", "B", "C"], data_size)}
    )

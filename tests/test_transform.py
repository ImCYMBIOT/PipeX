...
def test_transform_data(mocker):
    mocker.patch(
        "builtins.open", mocker.mock_open(read_data='def transform(data): data["column3"] = data["column1"] * 2; return data')
    )
    ...
    options = {"drop_columns": True, "rename_columns": False, "filter_rows": True, "add_columns": True}

    transformed_data = transform_data("tests/transform_script.py", config, data, options)

    expected_data = pd.DataFrame({"new_column1": [2, 3], "column3": [4, 6], "column4": [12, 13]})

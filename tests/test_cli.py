...
def sample_config():
    ...
    return {
        "extract": {
            "source": "api",
            "connection_details": {"headers": {"Authorization": "Bearer test-token"}},
        ...
        "transform": {
            ...
            "options": {"drop_columns": True, "rename_columns": True, "filter_rows": True, "add_columns": True},
        },
        "load": {
            "target": "S3 Bucket",
            "config": {
...
def test_extract_command_success(runner, temp_config_file, sample_data):
    ...
    with patch("app.cli.extract_data") as mock_extract:
        ...
        result = runner.invoke(app, ["extract", "api", temp_config_file])
...
def test_extract_command_with_output_file(runner, temp_config_file, sample_data):
    ...
    with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as output_file:
        with patch("app.cli.extract_data") as mock_extract:
            ...
            result = runner.invoke(app, ["extract", "api", temp_config_file, "--output", output_file.name])
...
def test_extract_command_missing_config(runner):
    ...
    result = runner.invoke(app, ["extract", "api", "nonexistent_config.yaml"])
...
def test_transform_command_success(runner, temp_config_file, sample_data):
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as input_file:
        ...
        with patch("app.cli.transform_data") as mock_transform:
            ...
            result = runner.invoke(app, ["transform", "tests/transform_script.py", temp_config_file, input_file.name])
...
def test_load_command_success(runner, temp_config_file, sample_data):
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as input_file:
        ...
        with patch("app.cli.load_data") as mock_load:
            result = runner.invoke(app, ["load", "S3 Bucket", temp_config_file, input_file.name])
...
def test_run_command_success(runner, temp_config_file, sample_data):
    ...
    with (
        patch("app.cli.extract_data") as mock_extract,
        ...
        result = runner.invoke(app, ["run", "--config", temp_config_file])
...
def test_run_command_dry_run(runner, temp_config_file):
    ...
    result = runner.invoke(app, ["run", "--config", temp_config_file, "--dry-run"])
...
def test_run_command_verbose(runner, temp_config_file, sample_data):
    ...
    with (
        ...
        result = runner.invoke(app, ["run", "--config", temp_config_file, "--verbose"])
